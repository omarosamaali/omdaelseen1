...
<div style="display: flex; flex-direction: row-reverse; margin-right: 77px !important; position: relative; margin: 0px 20px; z-index: 9999999999999;">
    <div style="height: fit-content; background: white; border-radius: 10px; padding: 20px; margin-top: 24px; width: 400px;">
        <div>
            ...
            عدد الأماكن
            <br />
            {{ $explorer->places_count ?? '0' }}
            <br />
            تاريخ الإضافة
            <br />
            {{ $explorer->created_at ? $explorer->created_at->format('Y-m-d H:i:s') : '-' }}
            <br />
            تاريخ التحديث
            <br />
            {{ $explorer->updated_at ? $explorer->updated_at->format('Y-m-d H:i:s') : '-' }}
        </div>
    </div>
    <div class="container py-4 mx-auto max-w-4xl" style="position: relative; right: -50px; margin-top: 24px; background: white; border-radius: 10px; padding: 20px;">
        <h2 class="text-right mb-4 font-bold text-xl">تعديل بيانات التصنيف الرئيسي</h2>
        <form action="{{ route('admin.explorers.update', $explorer->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2" style="column-gap: 20px;">
                <div class="mb-4">
                    ...
                    <input type="text" id="name_ar" name="name_ar" class="input-field @error('name_ar') border-red-500 @enderror" value="{{ old('name_ar', $explorer->name_ar) }}" required>
                    @error('name_ar')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="name_en" name="name_en" class="input-field @error('name_en') border-red-500 @enderror" value="{{ old('name_en', $explorer->name_en) }}" required>
                    @error('name_en')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="name_ch" name="name_ch" class="input-field @error('name_ch') border-red-500 @enderror" value="{{ old('name_ch', $explorer->name_ch) }}" required>
                    @error('name_ch')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="status" style="direction: ltr;" class="input-field @error('status') border-red-500 @enderror rtl-select">
                        ...
                        <option value="active" {{ old('status', $explorer->status) == 'active' || old('status', $explorer->status) == 'نشط' ? 'selected' : '' }}>نشط</option>
                        <option value="inactive" {{ old('status', $explorer->status) == 'inactive' || old('status', $explorer->status) == 'غير نشط' ? 'selected' : '' }}>غير نشط</option>
                    </select>
                    @error('status')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <div class="mt-2">
                        ...
                        <img src="{{ asset('storage/' . $explorer->avatar) }}" alt="{{ $explorer->name_ar }}" style="width: 91px; height: 63px;" class="mt-2 object-cover rounded">
                    </div>
                    @endif
                    @error('avatar')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            ...
            <div class="flex justify-between" style="margin-top: 20px;">
                <a href="{{ route('admin.explorers.index') }}" class="px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600">إلغاء</a>
