    ...
    <div style="">
        <div style="display: flex; flex-direction: row-reverse; justify-content: space-between;">
            ...
            <div
                ...
                <div>الأماكن المدخلة بواسطة {{ $user->name }}</div>
                    ...
                    <div style="position: relative;">
                        ...
                        <form method="GET" action="{{ route('admin.places.index') }}"
                            ...
                            <input aria-autocomplete="off" autocomplete="off" type="text" id="user_search"
                                name="user_search" value="{{ request('user_search') }}"
            ...
            <table id="places_table" class="w-full text-sm text-left rtl:text-right text-gray-500"
                ...
                <tbody>
                    ...
                    <tr class="odd:bg-white even:bg-gray-50 border-b border-gray-200">
                        <td class="th">
                            {{-- {{ $loop->iteration + $favorite->firstItem() - 1 }} --}}
                            1
                        </td>
                        <td class="th">
                            {{ $favorite->place->name_ar }}
                        </td>
                        <td class="th">
                            <img style="width: 40px; height: 40px; border-radius: 13px;"
                                src="{{ asset('storage/' . $favorite->place->avatar) }}" alt="{{ $favorite->place->name_ar }}">
                        </td>
                        <td class="th">
                            {{ $favorite->place->mainCategory ? $favorite->place->mainCategory->name_ar : 'غير محدد' }}
                        </td>
                        <td class="th">
                            {{ $favorite->place->subCategory ? $favorite->place->subCategory->name_ar : 'غير محدد' }}
                        </td>
                        <td class="th">
                            {{ $favorite->place->region ? $favorite->place->region->name_ar : 'غير محدد' }}
                        </td>
                        <td class="th">
                            <a href="{{ route('admin.fav_for_places.index', $favorite->place) }}"
                                style="padding-right: 5px; padding-left: 0px; display: flex; border-radius: 3px; text-align: center;">
                                {{ $ratingsData[$favorite->place->id]['ratings_count'] ?? 0 }} <svg
                        ...
                        <td class="th">
                            <a href="{{ route('admin.fav_places.index', $favorite->place) }}" style="display: flex; gap: 2px;">
                                {{ $favoritesCount[$favorite->place->id] ?? 0 }}
                        ...
                        <td class="th">
                            <a href="{{ route('admin.fav_for_places.index', $favorite->place) }}"
                                ...
                                {{ $commentsCount[$favorite->place->id] ?? 0 }}
                            </a>
                        </td>
                        <td class="th">
                            <span
                                class="{{ $favorite->place->status == 'active' ? 'text-green-600' : ($favorite->place->status == 'banned' ? 'text-red-600' : 'text-gray-600') }}">
                                {{ $favorite->place->status == 'active' ? 'نشط' : ($favorite->place->status == 'inactive' ? 'غير نشط' :
                                'محظور') }}
                            </span>
                        </td>
                        <td class="th" style="display: flex; ">
                            <a href="{{ route('admin.places.show', $favorite->place->id) }}" class="font-medium text-green-600">
                            ...
                            <a href="{{ route('admin.places.edit', $favorite->place->id) }}" class="font-medium text-blue-600">
                            ...
                            <form action="{{ route('admin.places.destroy', $favorite->place->id) }}"
