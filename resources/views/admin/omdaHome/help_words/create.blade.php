...
@section('content')
    <div class="py-4 text-end" style="margin-top: 30px; max-width: 800px; margin-right: auto; margin-left: auto;">
        <h2 class="text-2xl font-bold mb-4 text-right">إضافة كلمة مساعدة</h2>
        <form action="{{ route('admin.help_words.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="word_type" id="word_type"
                    ...
                    <option value="تحية وتعريف"
                        {{ old('word_type', $helpWord->word_type ?? '') == 'تحية وتعريف' ? 'selected' : '' }}>التحية
                        والتعريف بالنفس</option>
                    <option value="أسئلة" {{ old('word_type', $helpWord->word_type ?? '') == 'أسئلة' ? 'selected' : '' }}>
                        أسئلة</option>
                    <option value="طلب أو تقديم مساعدة"
                        {{ old('word_type', $helpWord->word_type ?? '') == 'طلب أو تقديم مساعدة' ? 'selected' : '' }}>طلب أو
                        تقديم مساعدة</option>
                    <option value="السفر" {{ old('word_type', $helpWord->word_type ?? '') == 'السفر' ? 'selected' : '' }}>
                        السفر</option>
                    <option value="التسوق" {{ old('word_type', $helpWord->word_type ?? '') == 'التسوق' ? 'selected' : '' }}>
                        التسوق</option>
                </select>
                @error('word_type')
            ...
            <div class="mb-4">
                <label for="word_ar" class=" text-right block text-sm font-medium text-gray-700">الكلمة (عربي)</label>
                <input type="text" name="word_ar" id="word_ar"
            ...
            <div class="mb-4">
                <label for="word_en" class=" text-right block text-sm font-medium text-gray-700">الكلمة (إنجليزي)</label>
                <input type="text" name="word_en" id="word_en"
            ...
            <div class="mb-4">
                <label for="word_zh" class=" text-right block text-sm font-medium text-gray-700">الكلمة (صيني)</label>
                <input type="text" name="word_zh" id="word_zh"
