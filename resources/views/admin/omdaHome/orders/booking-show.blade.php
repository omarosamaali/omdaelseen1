...
<div class="container mx-auto py-8 px-4">
        ...
        <div class="lg:col-span-2 bg-white rounded-lg shadow-md p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <div class="info-value font-mono">{{ $trip_request->order_number ?? '-' }}</div>
                </div>
                <div>
                    ...
                    <div class="info-value">
                        <span
                            ...
                            {{ $trip_request->payment_status == 'paid' ? 'مدفوعة' : 'غير مدفوعة' }}
                        </span>
                    </div>
                </div>
                <div>
    ...
    {{-- قسم تفاصيل الحقول كاملة --}}
    <div class="bg-white rounded-lg shadow-md p-6">
        <h2 class="text-xl font-bold mb-4 text-gray-800">تفاصيل الحجز</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">

            {{-- <div>
                <div class="info-label">ID الرحلة</div>
                <div class="info-value">{{ $trip_request->trip_id ?? '-' }}</div>
            </div>

            <div>
                <div class="info-label">ID المستخدم</div>
                <div class="info-value">{{ $trip_request->user_id ?? '-' }}</div>
            </div> --}}

            <div>
                <div class="info-label">تاريخ الحجز</div>
                <div class="info-value">{{ $trip_request->booking_date ? $trip_request->booking_date->format('Y-m-d
                    H:i') : '-' }}</div>
            </div>

            <div>
                <div class="info-label">نوع الطلب</div>
                <div class="info-value">{{ $trip_request->order_type ?? '-' }}</div>
            </div>

            <div>
                <div class="info-label">العنوان</div>
                <div class="info-value">{{ $trip_request->destination ?? '-' }}</div>
            </div>

            <div>
                <div class="info-label">اسم العميل</div>
                <div class="info-value">{{ $trip_request->customer_name ?? '-' }}</div>
            </div>

            {{-- <div>
                <div class="info-label">حالة الحجز</div>
                <div class="info-value">{{ $trip_request->status ?? '-' }}</div>
            </div> --}}

            <div>
                <div class="info-label">المبلغ</div>
                <div class="info-value">{{ number_format($trip_request->amount, 2) ?? '-' }}</div>
            </div>

            <div class="md:col-span-2">
                <div class="info-label">Payment Intent ID</div>
                <div class="info-value font-mono mb-0" style="margin-bottom: 0px;">{{ $trip_request->payment_intent_id
                    ?? '-' }}</div>
                <p class="note">رقم الطلب الخاص بك لتتبع الدفع عبر بوابة Ziina.</p>
                <p class="note">ملحوظة هذا الرقم يتم توليده من بوابة الدفع Ziina ليس من الموقع</p>
            </div>
