...
<div style="display: flex; flex-direction: row-reverse; margin: 0 20px;">
    <div class="container py-4 mx-auto max-w-4xl"
        ...
        <h2 class="text-right mb-4 font-bold text-xl">إضافة ملاحظة جديدة</h2>
...
@endif
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="mb-4">
                    ...
                    <input type="text" id="note_number" name="note_number"
                        ...
                        value="{{ old('note_number', default: $noteNumber) }}" required>
                    @error('note_number')
                ...
                <div class="mb-4">
                    ...
                    <input type="date" id="note_date" name="note_date"
                        ...
                        value="{{ old('note_date', now()->format('Y-m-d')) }}" required>
                    @error('note_date')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mb-4">
                ...
                <select name="status" class="input-field @error('status') border-red-500 @enderror"
                    ...
                    <option value="عامة" {{ old('status')=='عامة' ? 'selected' : '' }}>عامة</option>
                    <option value="خاصة" {{ old('status')=='خاصة' ? 'selected' : '' }}>خاصة</option>
