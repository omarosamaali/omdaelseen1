...
<div class="p-4 text-end" style="margin-top: 30px;">
    <div style="display: flex; flex-direction: row-reverse; justify-content: space-between;">
        <div style="margin-left: 20px;">
            <a href="{{ route('admin.orders.createDocument', $order->id) }}"
        ...
        <div style="display: flex; align-items: center; justify-content: space-between; gap:10px; margin-right: 20px;">
            ...
            <div style="display: flex; align-items: center; gap: 10px;">
                <form id="filter-form" action="{{ route('admin.orders.document', $order->id) }}" method="GET"
        ...
        <table id="explorers_table" class="w-full text-sm text-left rtl:text-right text-gray-500"
            ...
            <tbody>
                ...
                <tr class="odd:bg-white even:bg-gray-50 border-b border-gray-200">
                    <td class="th">{{ $document->document_number }}</td>
                    <td class="th">{{ $order->reference_number }}</td>
                    <td class="th">{{ $document->document_date }}</td>
                    <td class="th">{{ $document->title }}</td>
                    <td class="th">{{ $document->user->name ?? $document->user->email ?? 'غير متاح' }}</td>
                    <td class="th" style="display: flex; gap: 8px;">
                        <a href="{{ route('admin.orders.showDocument', $document->id) }}"
                        ...
                        <a href="{{ route('admin.orders.editDocument', $document->id) }}"
                            class="font-medium text-green-600">
                            <i class="fa-solid fa-edit" style="font-size: 18px;"></i>
                        </a>
                        <form action="{{ route('admin.orders.destroyDocument', $document->id) }}" method="POST"
                            style="display: inline;" onsubmit="return confirm('هل أنت متأكد من حذف هذا المستند؟');">
                            @csrf
                            @method('DELETE')
