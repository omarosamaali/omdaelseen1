...
<div style="display: flex; flex-direction: row-reverse; margin: 0 20px;">
    <div class="container py-4 mx-auto max-w-4xl"
        ...
        <h2 class="text-right mb-4 font-bold text-xl">تعديل الموافقة</h2>
        <form action="{{ route('admin.orders.updateApproval', $approval->id) }}" method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="mb-4">
                    ...
                    <input type="text" id="approval_number" name="approval_number"
                        ...
                        value="{{ old('approval_number', $approval->approval_number) }}" required>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" id="approval_date" name="approval_date"
                        ...
                        value="{{ old('approval_date', $approval->approval_date) }}" required>
                    @error('approval_date')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="title" name="title"
                        ...
                        value="{{ old('title', $approval->title) }}" required>
                    @error('title')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea rows="4" id="details" name="details"
                        ...
                        required>{{ old('details', $approval->details) }}</textarea>
                    @error('details')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <p class="text-sm text-gray-600 mt-2">الملف الحالي: <a
                            href="{{ Storage::url($approval->file_path) }}" target="_blank">عرض الملف</a></p>
                    @endif
                    @error('file')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
