...
<div style="display: flex; flex-direction: row-reverse; margin: 0 20px;">
    <div class="container py-4 mx-auto max-w-4xl"
        ...
        <h2 class="text-right mb-4 font-bold text-xl">تعديل الملاحظة</h2>
        <form action="{{ route('admin.orders.updateNote', $note->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="mb-4">
                    ...
                    <input type="text" id="note_number" name="note_number"
                        ...
                        value="{{ old('note_number', $note->note_number) }}" required>
                    @error('note_number')
                ...
                <div class="mb-4">
                    ...
                    <input type="date" id="note_date" name="note_date"
                        ...
                        value="{{ old('note_date', $note->note_date) }}" required>
                    @error('note_date')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    @if ($note->file_path)
                    <p class="text-sm text-gray-600 mt-2">الملف الحالي: <a href="{{ Storage::url($note->file_path) }}"
                            target="_blank">عرض الملف</a></p>
                    @endif
                    @error('file')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <select name="status" class="input-field @error('status') border-red-500 @enderror"
                        ...
                        <option value="عامة" {{ old('status', $note->status) == 'عامة' ? 'selected' : '' }}>عامة
                        </option>
                        <option value="خاصة" {{ old('status', $note->status) == 'خاصة' ? 'selected' : '' }}>خاصة
                        </option>
                        <option value="ملغية" {{ old('status', $note->status) == 'ملغية' ? 'selected' : '' }}>ملغية
                        </option>
                    </select>
                    @error('status')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
