...
<div style="display: flex; flex-direction: row-reverse; margin: 0 20px;">
    <div class="container py-4 mx-auto max-w-4xl"
        ...
        <h2 class="text-right mb-4 font-bold text-xl">تعديل ملاحظة الشحن</h2>
        <form action="{{ route('admin.orders.updateShippingNote', $shippingNote->id) }}" method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700">رقم الشحن</label>
                    <input type="text" id="note_number" name="note_number"
                        class="input-field @error('note_number') border-red-500 @enderror"
                        value="{{ old('note_number', $shippingNote->note_number) }}" required>
                    @error('note_number')
                ...
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700">تاريخ الشحن</label>
                    <input type="date" id="note_date" name="note_date"
                        class="input-field @error('note_date') border-red-500 @enderror"
                        value="{{ old('note_date', $shippingNote->note_date) }}" required>
                    @error('note_date')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="title" name="title"
                        ...
                        value="{{ old('title', $shippingNote->title) }}" required>
                    @error('title')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea rows="4" id="details" name="details"
                        ...
                        required>{{ old('details', $shippingNote->details) }}</textarea>
                    @error('details')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <p class="text-sm text-gray-600 mt-2">الملف الحالي: <a
                            href="{{ Storage::url($shippingNote->file_path) }}" target="_blank">عرض الملف</a></p>
                    @endif
                    @error('file')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <select name="status" class="input-field @error('status') border-red-500 @enderror"
                        ...
                        <option value="التجهيز للشحن" {{ old('status', $shippingNote->status) == 'التجهيز للشحن' ?
                            'selected' : '' }}>التجهيز للشحن</option>
                        <option value="تم الشحن" {{ old('status', $shippingNote->status) == 'تم الشحن' ? 'selected' : ''
                            }}>تم الشحن</option>
                    </select>
                    @error('status')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
