...
<div class="p-4 text-end" style="margin-top: 30px;">
    <div style="display: flex; flex-direction: row-reverse; justify-content: space-between;">
        <div style="margin-left: 20px;">
            <a href="{{ route('admin.orders.createShippingNote', $order->id) }}"
        ...
        <div style="display: flex; align-items: center; justify-content: space-between; gap:10px; margin-right: 20px;">
            ...
            <div style="display: flex; align-items: center; gap: 10px;">
                <form id="filter-form" action="{{ route('admin.orders.shippingNote', $order->id) }}" method="GET"
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg" style="max-width: 100%; margin: 20px">
        <table id="explorers_table" class="w-full text-sm text-left rtl:text-right text-gray-500"
            ...
            <thead class="text-xs text-gray-700 uppercase">
                <tr>
                    <th scope="col" class="th">رقم الشحن</th>
            ...
            <tbody>
                @foreach ($shippingNotes as $shippingNote)
                <tr class="odd:bg-white even:bg-gray-50 border-b border-gray-200">
                    <td class="th">{{ $shippingNote->note_number }}</td>
                    <td class="th">{{ $order->reference_number }}</td>
                    <td class="th">{{ $shippingNote->note_date }}</td>
                    <td class="th">{{ $shippingNote->title }}</td>
                    <td class="th">{{ $shippingNote->status }}</td>
                    <td class="th" style="display: flex; gap: 8px;">
                        <a href="{{ route('admin.orders.showShippingNote', $shippingNote->id) }}"
                        ...
                        <a href="{{ route('admin.orders.editShippingNote', $shippingNote->id) }}"
                            class="font-medium text-green-600">
                            <i class="fa-solid fa-edit" style="font-size: 18px;"></i>
                        </a>
                        <form action="{{ route('admin.orders.destroyShippingNote', $shippingNote->id) }}" method="POST"
                            style="display: inline;"
                            onsubmit="return confirm('هل أنت متأكد من حذف ملاحظة الشحن هذه؟');">
                            @csrf
                            @method('DELETE')
