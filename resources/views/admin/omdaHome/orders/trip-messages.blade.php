...
@section('title', 'عمدة الصين | Trip Chat')
            ...
            <div class="flex justify-start items-center gap-4">
                <div class="text-white">
                    <p style="font-size: 0.75rem; margin: auto;" class="text-center font-semibold">
                        {{ $trip->reference_number ?? 'غير متوفر' }}</p>
    ...
    document.getElementById('message-form').addEventListener('submit', function (e) {
        ...
        .then(data => {
            ...
            if (data.message.message) {
                messageContent += `<p class="text-color5 p-4 bg-white dark:bg-color9 dark:text-white rounded-l-2xl rounded-tr-2xl text-xs max-w-[280px]">${data.message.message}</p>`;
            }
            let newMessage = `
