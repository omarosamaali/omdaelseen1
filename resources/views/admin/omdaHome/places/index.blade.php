...
<div class="py-4 text-end" style="margin-top: 30px;">
    ...
    <div style="">
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg" style="max-width: 100%; margin: 20px">
            <table id="places_table" class="w-full text-sm text-left rtl:text-right text-gray-500" style="background: #c9c9c9;">
                ...
                <tbody>
                    ...
                    <tr class="odd:bg-white even:bg-gray-50 border-b border-gray-200">
                        ...
                        <td class="th">
                            <img style="width: 40px; height: 40px; border-radius: 13px;" src="{{ asset('storage/' . $place->avatar) }}" alt="{{ $place->name_ar }}">
                        </td>
                        <td class="th">
                        ...
                        <td class="th">
                            <div style="padding-right: 5px; padding-left: 0px; display: flex;  border-radius: 3px; text-align: center;">
                               {{ $ratingsData[$place->id]['ratings_count'] ?? 0 }} <svg class="w-5 h-5 text-yellow-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <td class="th">
                            <span class="{{ $place->status == 'active' ? 'text-green-600' : ($place->status == 'banned' ? 'text-red-600' : 'text-gray-600') }}">
                                {{ $place->status == 'active' ? 'نشط' : ($place->status == 'inactive' ? 'غير نشط' : 'محظور') }}
                            </span>
                        </td>
                        <td class="th" style="display: flex; ">
                            ...
                            <form action="{{ route('admin.places.destroy', $place->id) }}" onclick="return confirm('هل انت متاكد من حذف هذا')" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
            ...
            @if (!($places->count() > 0))
            <div style="padding-top: 20px; text-align: center; display: flex; align-items: center; justify-content: center; margin: auto;">
