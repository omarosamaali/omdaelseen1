...
@section('content')
<h2 class="text-right mb-4 font-bold text-xl" style="display: flex; margin-top: 50px; margin-right: 50px; ">تعديل سياسة الخصوصية</h2>
    ...
    <div style="display: flex; flex-direction: row-reverse; margin: 0px 20px; position: relative; z-index: 1;">
        <div class="container py-4 mx-auto max-w-4xl"
            ...
            <form action="{{ route('admin.privacy.update', $work->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="editor-wrapper">
                            ...
                            <div class="toolbar">
                                ...
                                <div class="toolbar-group" style="display: none;">>
                                    <button type="button" onclick="document.getElementById('fileInput_ar').click()">رفع صورة</button>
                                    <input type="file" id="fileInput_ar" class="file-input" accept="image/*">
                                </div>
                                <div class="toolbar-group" style="display: none;">>
                            ...
                            <div class="editor-container">
                                ...
                                <div class="editor-content" id="editor_ar" contenteditable="true" data-lang="ar">{!! $work->content_ar ?? '' !!}</div>
                            </div>
                        ...
                        <input type="hidden" name="content_ar" id="content_ar" value="{!! $work->content_ar ?? '' !!}">
                        @error('content_ar')
                    ...
                    <div class="mb-4">
                        ...
                        <div class="editor-wrapper">
                            ...
                            <div class="toolbar">
                                ...
                                <div class="toolbar-group" style="display: none;">
                                    <button type="button" onclick="document.getElementById('fileInput_en').click()">Upload Image</button>
                                    <input type="file" id="fileInput_en" class="file-input" accept="image/*">
                                </div>
                                <div class="toolbar-group" style="display: none;">
                                    <button type="button" onclick="triggerManualTranslation('editor_en', 'en', ['ar', 'zh-CN'], [$('#editor_ar')[0], $('#editor_ch')[0]])">Translate Manually</button>
                                </div>
                            </div>
                            <div class="editor-container">
                                <div class="line-numbers" id="lineNumbers_en"></div>
                                <div class="editor-content" id="editor_en" contenteditable="true" data-lang="en" style="direction: ltr; text-align: left;">{!! $work->content_en ?? '' !!}</div>
                            </div>
                            <div class="status-bar">
                                <span id="charCount_en">Characters: 0</span>
                                <span id="wordCount_en">Words: 0</span>
                            </div>
                        </div>
                        <input type="hidden" name="content_en" id="content_en" value="{!! $work->content_en ?? '' !!}">
                        @error('content_en')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="editor-wrapper">
                            ...
                            <div class="toolbar">
                                ...
                                <div class="toolbar-group" style="display: none;">
                                    <button type="button" onclick="document.getElementById('fileInput_ch').click()">上传图片</button>
                                    <input type="file" id="fileInput_ch" class="file-input" accept="image/*">
                                </div>
                                <div class="toolbar-group" style="display: none;"> 
                                    <button type="button" onclick="triggerManualTranslation('editor_ch', 'zh-CN', ['ar', 'en'], [$('#editor_ar')[0], $('#editor_en')[0]])">手动部的翻译按钮</button>
                                </div>
                            </div>
                            <div class="editor-container">
                                <div class="line-numbers" id="lineNumbers_ch"></div>
                                <div class="editor-content" id="editor_ch" contenteditable="true" data-lang="zh-CN" style="direction: ltr; text-align: left;">{!! $work->content_zh ?? '' !!}</div>
                            </div>
                            <div class="status-bar">
                                <span id="charCount_ch">字符数: 0</span>
                                <span id="wordCount_ch">单词数: 0</span>
                            </div>
                        </div>
                        <input type="hidden" name="content_zh" id="content_zh" value="{!! $work->content_zh ?? '' !!}">
                        @error('content_zh')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="status" style="direction: ltr;" class="input-field @error('status') border-red-500 @enderror rtl-select">
                            ...
                            <option value="نشط" {{ old('status', $work->status) == 'نشط' ? 'selected' : '' }}>نشط</option>
                            <option value="غير نشط" {{ old('status', $work->status) == 'غير نشط' ? 'selected' : '' }}>غير نشط</option>
                        </select>
                        @error('status')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        @if ($work->avatar)
                            <div class="mt-2">
                                ...
                                <img src="{{ asset('storage/' . $work->avatar) }}" alt="Work Image" style="width: 91px; height: 63px;" class="mt-2 object-cover rounded">
                            </div>
                        @endif
                        @error('avatar')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div class="flex justify-between" style="margin-top: 20px;">
                    <a href="{{ route('admin.works.index') }}" class="px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600">إلغاء</a>
    ...
    <script>
        ...
        function updateStats(editorId) {
            ...
            charCount.textContent = `عدد الأحرف: ${text.length}`;
