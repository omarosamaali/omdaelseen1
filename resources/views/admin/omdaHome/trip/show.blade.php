    ...
    <div class="bg-white rounded-lg shadow-lg p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="bg-gray-100 p-4 rounded-lg">
                ...
                <p class="text-lg font-medium text-gray-900 mt-1">{{ $tripGuideline->name_ar }}</p>
            </div>
            
            <div class="bg-gray-100 p-4 rounded-lg">
                <p class="text-sm font-semibold text-gray-600">الإرشاد بالإنجليزي:</p>
                <p class="text-lg font-medium text-gray-900 mt-1">{{ $tripGuideline->name_en ?? 'لا يوجد' }}</p>
            </div>
            
            <div class="bg-gray-100 p-4 rounded-lg">
                <p class="text-sm font-semibold text-gray-600">الإرشاد بالصيني:</p>
                <p class="text-lg font-medium text-gray-900 mt-1">{{ $tripGuideline->name_ch ?? 'لا يوجد' }}</p>
            </div>
            
            <div class="bg-gray-100 p-4 rounded-lg">
                <p class="text-sm font-semibold text-gray-600">الحالة:</p>
                <p class="text-lg font-medium text-gray-900 mt-1">
                    {{ $tripGuideline->status == 'active' ? 'نشط' : 'غير نشط' }}
                </p>
            </div>
            
            <div class="bg-gray-100 p-4 rounded-lg">
                <p class="text-sm font-semibold text-gray-600">الترتيب:</p>
                <p class="text-lg font-medium text-gray-900 mt-1">{{ $tripGuideline->order }}</p>
            </div>
            
            <div class="bg-gray-100 p-4 rounded-lg">
                <p class="text-sm font-semibold text-gray-600">تاريخ الإنشاء:</p>
                <p class="text-lg font-medium text-gray-900 mt-1">{{ $tripGuideline->created_at->format('Y-m-d H:i') }}</p>
            </div>
            <div class="bg-gray-100 p-4 rounded-lg">
                <p class="text-sm font-semibold text-gray-600">آخر تحديث:</p>
                <p class="text-lg font-medium text-gray-900 mt-1">{{ $tripGuideline->updated_at->format('Y-m-d H:i') }}</p>
            </div>
        </div>

        <div class="flex justify-end mt-8 gap-2">
            <a href="{{ route('admin.omdaHome.trip.edit', $tripGuideline->id) }}" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 transition duration-200">تعديل</a>
