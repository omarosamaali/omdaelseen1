    ...
    <div class="py-4 text-end" style="margin-top: 30px;">
        <div style="">
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg" style="max-width: 100%; margin: 20px">
                <table id="interests_table" class="w-full text-sm text-left rtl:text-right text-gray-500"
                    ...
                    <tbody>
                        @foreach ($userInterests as $userInterest)
                            <tr class="odd:bg-white even:bg-gray-50 border-b border-gray-200">
                                ...
                                @php $counter++; @endphp
                                <td class="th">{{ $userInterest->user->name ?? 'غير معروف' }}</td>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                searchInput.addEventListener('input', function() {
                    ...
                    rows.forEach(row => {
                        const userCell = row.querySelectorAll('td')[1]; // User name
                        const itemCell = row.querySelectorAll('td')[3]; // Interest item
                        if (!userCell || !itemCell) return;

                        const user = userCell.innerText.trim().toLowerCase();
                        const item = itemCell.innerText.trim().toLowerCase();

                        if (user.includes(term) || item.includes(term) || term === '') {
                            row.style.display = '';
                        } else {
