...
<body class="relative -z-20">
    <div class="container h-screen dark:text-white flex flex-col justify-between">
        <div class="flex justify-between items-center bg-p2 px-6 py-6" style="position: fixed; width: 100%; z-index: 999999999999999;">
            <div class="flex justify-start items-center gap-4">
                ...
                <div style="background-color: white;" class="relative border-2 border-p1 p-1 rounded-full">
                    <img src="{{ $chatUser->avatar ? asset('storage/' . $chatUser->avatar) : asset('assets/images/user-img-1.png') }}"
                        alt="{{ $chatUser->name }}" class="size-10 rounded-full bg-color8" />
                    <div class="bg-white p-0.5 rounded-full absolute -right-1.5 bottom-1">
                        <div class="size-3 rounded-full bg-p3"></div>
                    </div>
                </div>
                <div class="text-white">
                    <p class="text-2xl font-semibold">{{ $chatUser->name }}</p>
                ...
                <div class="flex justify-start items-start gap-3 w-full px-6">
                    <div class="flex flex-col justify-center items-center shrink-0">
                        <div class="relative border-2 border-p1 p-1 rounded-full">
                            <img src="{{ $message->sender->avatar ? asset('storage/' . $message->sender->avatar) : asset('assets/images/user-img-1.png') }}"
                                alt="{{ $message->sender->name }}" class="size-9 rounded-full bg-color8" />
                            <div class="bg-white p-0.5 rounded-full absolute -right-1.5 bottom-1">
                                <div class="size-3 rounded-full bg-p3"></div>
                            </div>
                        </div>
    ...
    <script>
        console.log('Chat User ID from Blade on load:', {{ $chatUser->id }});
        console.log('Chat User Name from Blade on load:', '{{ $chatUser->name }}');
            ...
            .then(text => {
                try {
                    ...
                    if (data.message.image) {
                        messageContent += `<img src="{{ asset('storage/') }}/${data.message.image}" alt="Message Image" class="max-h-[200px] max-w-[200px] rounded-lg border border-color21" />`;
                    }
                    ...
                    alert('حدث خطأ أثناء إرسال الرسالة: ' + text);
                }
            })
