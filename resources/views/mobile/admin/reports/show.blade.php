    ...
    <div class="container mx-auto px-4 py-6 max-w-md">
                    ...
                    <div class="images-gallery">
                        @foreach (json_decode($report->place->additional_images, true) as $image)
                            <img src="{{ asset('storage/' . $image) }}"
                                alt="{{ $report->place->name_ar ?? 'مكان غير متوفر' }}" class="place-image">
                        @endforeach
                    </div>
                @endif
            </div>
        ...
        @if ($report->status == 'pending')
            <div class="actions-container">
                ...
                <form method="POST" action="{{ route('mobile.reports.accept', $report->id) }}">
                    @csrf
                    <button type="submit" class="action-btn btn-accept"
                        onclick="return confirm('هل أنت متأكد من قبول البلاغ وتعطيل المكان؟\n\nسيتم تعطيل المكان فوراً ولن يظهر للمستخدمين.')">
                        <i class="fas fa-check ml-2"></i>
                        قبول البلاغ وتعطيل المكان
                    </button>
                </form>
                <form method="POST" action="{{ route('mobile.reports.dismiss', $report->id) }}">
                    @csrf
                    <button type="submit" class="action-btn btn-dismiss"
                        onclick="return confirm('هل أنت متأكد من رفض البلاغ؟\n\nسيبقى المكان نشطاً وسيتم إغلاق البلاغ.')">
