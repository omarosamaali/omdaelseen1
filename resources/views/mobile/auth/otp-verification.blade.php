...
<div class="container min-h-dvh relative overflow-hidden py-8 px-6 dark:text-white dark:bg-color1">
    ...
    <div class="flex justify-start items-center relative z-10">
        ...
        <x-back-button href="{{ route('mobile.auth.forgot-password') }}" />
    </div>
    <form action="{{ route('mobile.auth.otp.verify') }}" method="POST" class="relative z-10 otp-form">
        @csrf
        <div class="bg-white py-8 px-6 rounded-xl mt-12 dark:bg-color10">
            <div class="flex flex-col gap-3 text-center">
                ...
                <p class="text-bgColor18 text-sm dark:text-color18">
                    ...
                    <span class="text-p1">{{ session('email') }}</span>
                    <input type="hidden" name="email" value="{{ session('email') }}">
                </p>
            </div>
            <div class="py-6">
                <input type="text" name="otp" class="item otp-form-item text-center" style="width: 100% !important;" maxlength="6" required />
            </div>
            <p class="text-bgColor18 text-sm dark:text-white pt-2 text-center">
                لم يصلك رمز التحقق ؟
                <a href="{{ route('mobile.auth.forgot-password') }}" class="text-p1">إعادة إرسال</a>
            </p>
        </div>
