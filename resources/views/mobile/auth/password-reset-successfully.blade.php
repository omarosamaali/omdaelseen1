@extends('layouts.mobile')

@section('title', 'إستعادة كلمة المرور | Forgot Password')

@section('content')
<main class="container bg-white min-h-screen dark:bg-black dark:text-white">
    <div class="flex justify-center items-center flex-col h-screen px-6">
        <div class="wave-animation">
            <div class="size-32 rounded-full bg-g60 flex justify-center items-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" fill="white" width="90" height="90">
                    <path stroke="#ffffff" d="M144 224C161.7 224 176 238.3 176 256L176 512C176 529.7 161.7 544 144 544L96 544C78.3 544 64 529.7 64 512L64 256C64 238.3 78.3 224 96 224L144 224zM334.6 80C361.9 80 384 102.1 384 129.4L384 133.6C384 140.4 382.7 147.2 380.2 153.5L352 224L512 224C538.5 224 560 245.5 560 272C560 291.7 548.1 308.6 531.1 316C548.1 323.4 560 340.3 560 360C560 383.4 543.2 402.9 521 407.1C525.4 414.4 528 422.9 528 432C528 454.2 513 472.8 492.6 478.3C494.8 483.8 496 489.8 496 496C496 522.5 474.5 544 448 544L360.1 544C323.8 544 288.5 531.6 260.2 508.9L248 499.2C232.8 487.1 224 468.7 224 449.2L224 262.6C224 247.7 227.5 233 234.1 219.7L290.3 107.3C298.7 90.6 315.8 80 334.6 80z" />
                </svg>
            </div>
            <div class="waves wave-1"></div>
            <div class="waves wave-2"></div>
            <div class="waves wave-3"></div>
        </div>
        <div class="flex justify-center items-center flex-col py-20" style="z-index: 9;">
            <h1 class="text-2xl font-semibold text-p2 dark:text-p1">
                نجاح!
            </h1>
            <p class="text-xs pt-3 px-4 text-center">
                تهانينا! لقد سجّلت دخولك بنجاح. استعد لاستكشاف تطبيقنا الشيق، وتعلّم، واستمتع به!
            </p>
        </div>
        <div class="w-full" style="z-index: 9;">
            <a href="{{ route('mobile.auth.login') }}" class="bg-p2 rounded-full py-3 text-white text-sm font-semibold text-center block mt-12 dark:bg-p1">تسجيل
                دخول</a>
        </div>
    </div>
</main>

@endsection
