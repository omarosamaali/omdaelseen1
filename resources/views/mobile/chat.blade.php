...
<body class="relative -z-20">
            ...
            <div class="flex flex-col gap-8 w-full" style="overflow: auto; margin-top: 53px;">
                ...
                <div class="flex justify-end items-end gap-3 w-full px-6">
                    <div class="flex flex-col gap-3">
                        <div class="flex justify-start items-center gap-2">
                            @if ($message->image)
                            <img style="max-height: 200px;" src="{{ asset('storage/' . $message->image) }}" alt="1Message Image"
                ...
                <div class="flex justify-start items-start gap-3 w-full px-6">
                    ...
                    <div class="flex flex-col gap-3">
                        <div class="flex justify-start items-center gap-2">
                            @if ($message->image)
                            <img src="{{ asset('storage/' . $message->image) }}" style="max-height: 200px;" alt="2Message Image"
    ...
    <script>
        console.log('Admin ID from Blade on load:', {{ $admin->id }});
        console.log('Admin Name from Blade on load:', '{{ $admin->name }}');
        ...
        document.getElementById('message-form').addEventListener('submit', function (e) {
            ...
            console.log('Sending message to receiver_id:', {{ $admin->id }});
            ...
            .then(text => {
                try {
                    ...
                    if (data.message.image) {
                        messageContent += `<img src="{{ asset('storage/') }}/${data.message.image}" alt="3Message Image" style="max-height: 200px;" class="max-w-[200px] rounded-lg border border-color21" />`;
                    }
