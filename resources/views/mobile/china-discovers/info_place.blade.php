...
<div class="container dark:text-white dark:bg-black">
    <div class="container--header">
        ...
        @if(!App\Models\ReviewReport::where('user_id', Auth::user()->id)->where('place_id', $place->id)->exists())
    ...
    <div class="container--features">
        <div>
            ...
            <p>{{ $place->region->name_ar }}</p>
        </div>
        <div>
            <img src="{{ asset('storage/' . $place->mainCategory->avatar) }}" alt="">
            <p>{{ $place->mainCategory->name_ar }}</p>
        </div>
        <div>
    ...
    <div class="tabs-container">
        <div class="tab-buttons">
            ...
            <button class="tab-button" onclick="showTab('tab2')">معرض الصور</button>
            <button class="tab-button rating" onclick="showTab('tab3')">التقييم</button>
        </div>
        <div id="tab3" class="tab-content">
            <div class="overall-rating">
                <div class="rating-summary">
                    <div class="average-rating">
                        ...
                        <span class="total-reviews" id="totalReviews">(0 تقييم)</span>
                    </div>
                </div>
            </div>
            <!-- Check if user can add a rating -->
            @if(auth()->check() && !App\Models\Rating::where('user_id', Auth::user()->id)->where('place_id', $place->id)->exists())
            <div class="add-rating-section">
                <h4 class="add-rating-title">أضف تقييمك</h4>
            ...
            <div class="already-rated-message">
                <p>لقد قيّمت هذا المكان بالفعل. شكرًا على تقييمك!</p>
            </div>
            @endif
            ...
            <div class="reviews-section">
                ...
                <h4 class="reviews-title">آراء الزوار</h4>
        ...
        <div id="tab1" class="tab-content">
            ...
            @if($place->phone)
            <p class="tab-details-content"><i class="fa-solid fa-phone" style="color: #3b3129;"></i> {{ $place->phone }}</p>
            @endif
            @if($place->email)
            <p class="tab-details-content"><i class="fa-solid fa-envelope" style="color: #3b3129;"></i> {{ $place->email }}</p>
            @endif
            <p class="tab-details-link"><a href="{{ $place->link }}"><i class="fa-solid fa-location-dot"></i> رابط المكان</a></p>
        </div>
    </div>
...
<script>
    ...
    function renderReviews() {
        ...
        if (allReviews.length === 0) {
            reviewsList.innerHTML = '<div class="no-reviews">لا توجد تقييمات حتى الآن. كن أول من يقيم هذا المكان!</div>';
            return;
        }
                ...
                if (data.message) {
                    ...
                    if (isEditing) {
                        ...
                        if (!existingMessage) {
                            ...
                            reviewsSection.insertBefore(alreadyRatedMessage, reviewsSection.querySelector('.reviews-title'));
                        }
                    }
    ...
    function resetModalForNewRating() {
        ...
        if (modalTitle) modalTitle.innerHTML = `<i class="fa fa-chat-circle-text" style="color: #ffb531;"></i> ما رأيك في هذا المكان؟`;
        isEditing = false;
        editingReviewId = null;
    }
