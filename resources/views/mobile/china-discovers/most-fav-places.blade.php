...
@section('content')
<x-china-header :title="__('messages.الأكثر تقييما')" :route="route('mobile.china-discovers.all--places')" />
<div class="container min-h-dvh relative pb-8 dark:text-white dark:bg-black"
    style="overflow-y: auto !important; padding-top: 30px;">
    @if($places->isNotEmpty())
    <div style="width: 100%; display: block;">
        @if ($banners->isNotEmpty())
        @foreach ($banners as $banner)
        <img class="fav-image" style="width: 100%; padding-top: 31px;" src="{{ asset('storage/' . $banner->avatar) }}"
            alt="">
        @endforeach
        @endif
    </div>
    @endif
    <div
        style="display: grid; grid-template-columns: repeat(1, 1fr); gap: 10px; margin: 0px 10px; height: 221px; justify-content: center;">
        @if($places->isNotEmpty())
