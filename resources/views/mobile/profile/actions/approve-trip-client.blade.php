...
@section('title', 'الموافقات | Approvals')
            ...
            <div class="flex flex-col gap-4 pt-8" style="direction: rtl;">
                ...
                <div class="shadow flex border justify-between items-center bg-white dark:bg-color9 py-4 px-5 rounded-2xl"
                    ...
                    <div class="flex justify-start items-center gap-3">
                        ...
                        @if (Auth::user()->role == 'admin')
                        <form id="deleteInvoiceForm-{{ $approve->id }}" action="{{ route('mobile.delete-approve', $approve ->id) }}"
                            method="POST" style="display: inline;">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete({{ $approve->id }})">
                    ...
                    <div
                        ...
                        <a
                            href="{{ route('mobile.profile.actions.show_approve-trip-client', ['tripId' => $trip->id, 'approveId' => $approve->id]) }}">
