...
@section('content')
<x-china-header :title="__('messages.edit_profile')" :route="route('mobile.profile.profile')" />
<div style="padding-top: 70px;"
    ...
    <form action="{{ route('mobile.profile.update') }}" method="POST" class="relative z-20"
            ...
            <div class="relative size-40 flex justify-center items-center" style="min-width: 132px;">
                <label for="avatar_input" class="cursor-pointer">
                    ...
                    <img src="{{ Storage::url(Auth::user()->avatar) }}" alt="User Avatar"
                        class="bg-[#B190B6] rounded-full overflow-hidden edit-img" />
                    @else
                    <img src="https://img.freepik.com/free-vector/businessman-character-avatar-isolated_24877-60111.jpg?t=st=1755956485~exp=1755960085~hmac=237ad4a2dca4c1c8b053fc5bf4abbc25b225c8142b452c30501fb28ac00abc5e&w=740"
                        alt="Default Avatar" class="bg-[#B190B6] rounded-full overflow-hidden edit-img" />
                    @endif
                    <span class="container--avatar">
                        <i style="top: 2px; justify-content: center; display: flex; z-index: 9999; position: relative;"
                            class="ph ph-camera text-xl text-bgColor18"></i>
                    </span>
                </label>
        ...
        <div class="bg-white py-8 px-6 rounded-xl dark:bg-color10">
            <div>
                <p class="text-sm font-semibold pb-2">{{ __('messages.name_label') }}</p>
                <div
            ...
            <div class="pt-4">
                <p class="text-sm font-semibold pb-2">{{ __('messages.email_label') }}</p>
                <div
                    class="flex justify-between items-center py-3 px-4 border border-color21 rounded-xl dark:border-color18 gap-3">
            ...
            <div class="pt-4">
                <p class="text-sm font-semibold pb-2">{{ __('messages.phone_label') }}</p>
                <div
                    class="flex justify-between items-center py-3 px-4 border border-color21 rounded-xl dark:border-color18 gap-3">
            ...
            <div class="pt-4">
                <p class="text-sm font-semibold pb-2">{{ __('messages.country_label') }}</p>
                <div
                    class="flex justify-between items-center py-3 px-4 border border-color21 rounded-xl dark:border-color18 gap-3">
                    <select name="country"
                        class="outline-none bg-transparent text-n600 text-sm placeholder:text-sm w-full placeholder:text-bgColor18 dark:text-color18 dark:placeholder:text-color18">
                        <option value="">{{ __('messages.country_placeholder') }}</option>
            ...
            <div class="pt-4">
                <p class="text-sm font-semibold pb-2">{{ __('messages.explorer_name_label') }}</p>
                <div
                    class="flex justify-between items-center py-3 px-4 border border-color21 rounded-xl dark:border-color18 gap-3">
                    <input type="text" value="{{ old('explorer_name', Auth::user()->explorer_name) }}"
                        name="explorer_name"
                        class="outline-none bg-transparent text-n600 text-sm placeholder:text-sm w-full placeholder:text-bgColor18 dark:text-color18 dark:placeholder:text-color18" />
                    <i class="fa fa-location-dot text-xl text-bgColor18 !leading-none"></i>
                </div>
                @error('explorer_name')
                <span class="text-red-500 text-xs mt-1">{{ __('messages.explorer_name_error') }}</span>
                @enderror
            </div>

            <div class="pt-4">
                <span style="color: red; font-size: 12px;">{{ __('messages.password_instruction') }}</span>
                <p class="text-sm font-semibold pb-2">{{ __('messages.password_label') }}</p>
                <div
                    class="flex justify-between items-center py-3 px-4 border border-color21 rounded-xl dark:border-color18 gap-3">
            ...
            <div class="pt-4">
                <p class="text-sm font-semibold pb-2">{{ __('messages.password_confirmation_label') }}</p>
                <div
                    class="flex justify-between items-center py-3 px-4 border border-color21 rounded-xl dark:border-color18 gap-3">
    ...
    <form id="delete-form" action="{{ route('mobile.profile.delete-account') }}" method="POST">
        @csrf
        @method('DELETE')
        <button type="button" onclick="showDeleteAlert()" style="width: 100%; margin-top: 20px; background-color: red;"
    ...
    <script>
        function showDeleteAlert() {
                Swal.fire({
                    ...
                    icon: 'warning',
                    input: 'text',
                    inputPlaceholder: '{{ __('messages.delete_input_placeholder') }}',
                    ...
                    preConfirm: (inputValue) => {
                        if (inputValue !== 'Delete') {
                            Swal.showValidationMessage(
                                '{{ __('messages.delete_validation_message') }}'
                            );
                        }
                    }
                }).then((result) => {
