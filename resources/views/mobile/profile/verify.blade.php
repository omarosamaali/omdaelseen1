...
<body>
    ...
    <div class="container min-h-dvh relative overflow-hidden py-8 px-6 dark:text-white dark:bg-color1">
        ...
        <div class="flex justify-start items-center relative z-10">
            ...
            <a href="{{ route('mobile.profile.profile') }}" class="bg-white p-2 rounded-full flex justify-center items-center text-xl dark:bg-color10">
        ...
        <form action="{{ route('mobile.profile.verify-otp') }}" method="POST" class="relative z-10 otp-form">
            ...
            <div class="bg-white py-8 px-6 rounded-xl mt-12 dark:bg-color10">
                <div class="flex flex-col gap-3 text-center">
                    <h3 class="text-xl font-semibold">أدخل رمز التأكيد</h3>
                    <p class="text-bgColor18 text-sm dark:text-color18">
                        أدخل رمز التحقق الذي تم إرساله بالفعل إلى
                ...
                <p class="text-bgColor18 text-sm dark:text-white pt-2 text-center" id="resendSection">
                    لم يصلك رمز التحقق؟ <span id="timer">01:00</span>
                </p>
            </div>
            <button type="submit" class="w-full bg-p2 rounded-full py-3 text-white text-sm font-semibold text-center block mt-12 dark:bg-p1">تأكيد</button>
        </form>
    </div>
    ...
    <script>
        function startTimer() {
            ...
            const timer = setInterval(function() {
                if (timeLeft <= 0) {
                    clearInterval(timer);
                    resendSection.innerHTML = 'لم يصلك رمز التحقق؟ <button type="button" onclick="resendOtp()" class="text-p1">إعادة إرسال</button>';
                } else {
