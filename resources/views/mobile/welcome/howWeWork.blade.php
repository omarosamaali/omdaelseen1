@extends('layouts.mobile')

@section('title', 'عمدة الصين | China Omda')

@section('content')
<style>
    .fa-question {
        transform: rotateY(190deg);
    }

    body {
        position: relative !important;
    }

    .tab-button .item.active {
        background-color: #000;
        color: #fff;
        padding: 8px 16px;
        border-radius: 9999px;
    }
</style>

<x-china-header :title="__('messages.how_we_work')" :route="route('mobile.welcome')" />

<body class="relative -z-20">
    <div class="container min-h-dvh relative overflow-hidden py-8 dark:text-white -z-10 dark:bg-color1">
        @if($howWeWork)
        <div class="relative z-10 px-6">
            <div class="flex flex-col justify-center items-center text-center">
                <img src="{{ asset('storage/'. $howWeWork?->avatar) }}" alt=""
                    style="padding-top:30px; height: 300px;" />
                <p class="text-color5 pt-5 px-6 dark:text-bgColor5">
                    {!! app()->getLocale() == 'en'
                    ? $howWeWork?->content_en
                    : (app()->getLocale() == 'zh'
                    ? $howWeWork?->content_zh
                    : $howWeWork?->content_ar) !!}
                </p>
            </div>
        </div>
        @else
        <div class="relative z-10 px-6" style="padding-top: 50px;">
            <x-empty />
        </div>
        @endif
    </div>

</body>
@endsection